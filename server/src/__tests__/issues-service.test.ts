...
describeEmbeddedPostgres("issueService.create idempotency", () => {
  let db!: ReturnType<typeof createDb>;
  let svc!: ReturnType<typeof issueService>;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;
  ...
  it("converges concurrent submissions to one issue and replays equivalent requests", async () => {
    const companyId = randomUUID();
    ...
    const request = {
      title: "Create this issue once",
      description: "The client may retry after a network cut.",
      status: "todo" as const,
      priority: "medium" as const,
      idempotencyKey,
    };
    const outcomes = await Promise.allSettled([
      svc.createWithResult(companyId, request),
      svc.createWithResult(companyId, request),
    ]);
    const fulfilled = outcomes.filter(
      (outcome): outcome is PromiseFulfilledResult<Awaited<ReturnType<typeof svc.createWithResult>>> =>
    ...
    expect(fulfilled[0]!.value.reused).toBe(false);

    const rows = await db
      .select({ id: issues.id, idempotencyKey: issues.idempotencyKey })
      .from(issues)
      .where(eq(issues.companyId, companyId));
    expect(rows).toEqual([{ id: fulfilled[0]!.value.issue.id, idempotencyKey }]);

    const replay = await svc.createWithResult(companyId, request);
    expect(replay.reused).toBe(true);
    expect(replay.issue.id).toBe(fulfilled[0]!.value.issue.id);

    await expect(
      svc.createWithResult(companyId, { ...request, title: "A different issue" }),
